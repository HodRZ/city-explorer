...
class App extends Component {
  ...
  handleSearch = async (cityName) => {
    ...
    const weatherData = await axios.get(`https://hod-city-explorer-api.herokuapp.com/weather?lon=${searchData[0].lon}&lat=${searchData[0].lat}&searchQuery=${cityName}`).catch(function (error) {
      Swal.fire({
        title: 'Sorry !',
    ...
    const movieData = await axios.get(`https://hod-city-explorer-api.herokuapp.com/movies?queryName=${cityName}`)
    this.setState({
