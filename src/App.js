...
class App extends Component {
  ...
  handleSearch = async (cityName) => {
    ...
    const weatherData = await axios.get(`${process.env.REACT_APP_PORT}/weather?lon=${searchData[0].lon}&lat=${searchData[0].lat}&searchQuery=${cityName}`).catch(function (error) {
      Swal.fire({
        title: 'Sorry !',
    ...
    const movieData = await axios.get(`${process.env.REACT_APP_PORT}/movies?queryName=${cityName}`)
    this.setState({
